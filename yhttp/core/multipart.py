...
This module completely excluded from code coverage because the 95% of code
coverage.
"""
...
def parse_form_data(environ, charset="utf8", strict=False, **kwargs):
    ...
    try:
        if environ.get("REQUEST_METHOD", "GET").upper() in VERBS_PROHIBITED:
            raise MultipartError(
                f"{', '.join(VERBS_PROHIBITED[:-1])} and VERBS_PROHIBITED[-1]"
                "verbs are not allowed to submit the HTTP body.")
